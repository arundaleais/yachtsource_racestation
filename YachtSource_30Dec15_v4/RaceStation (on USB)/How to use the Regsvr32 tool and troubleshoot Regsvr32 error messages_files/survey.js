...
if (!MS.Support.Fms.Survey) {
    ...
    MS.Support.Fms.Survey = function (id, config) {
        ...
        this.PreFillResponses = function () {
            ...
            var p9 = $xml.find("P9").text();
            if (p9) {
                var spans = $("label[spanvalue]").attr("spanvalue");
                $.each(spans.split(','),
                    function (no, span) {
                        //find the span and fill it with P9 fetched from xml
                        var span = document.getElementById(span);
                        while (span.firstChild) { span.removeChild(span.firstChild); }
                        span.appendChild(document.createTextNode(p9));
                        span.innerHtml = p9;
                    }
                    );
            }
