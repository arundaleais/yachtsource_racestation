   ...
   Begin VB.Frame fraMain 
      ...
      Begin VB.Frame fraLastStart 
         ...
         Begin VB.Label Label3 
            ...
            Width           =   735
         End
         Begin VB.Label lblTimeFromLastStart 
            AutoSize        =   -1  'True
            BackColor       =   &H80000014&
            Caption         =   "00:00"
            BeginProperty Font 
            ...
            Width           =   960
         End
         Begin VB.Label lblLastStartName 
            Caption         =   "None"
            Height          =   255
            Left            =   120
            TabIndex        =   19
...
'Unable to unload within this context (Error 365)
    ...
    If Cancel = True Then   'Called by the UnloadTimer to Cancel Terminate
        Cancel = False
        Exit Sub    'Because the Program Stae remains the same
    End If
    
    If cboProfile.ListIndex >= 0 Then
        CurrentProfile = cboProfile.List(cboProfile.ListIndex)
        Call GoProgramState(1) 'Load Profile
    Else
        Call GoProgramState(0) 'Loading Profile
    End If
End Sub
...
#End If
    Call GoProgramState(2) 'Profile Loaded
End Sub
...
'We must set the Recall States using Go...
            If State.Program >= 4 Then  '4=Start Sequence Commenced
...
'syc            Call GoLowerRecall
            Call RecallTimeout  'Must use to set Finish if last sequence
        End If
        Call GoToggleKey(KeyNo)
    Case Is = "General Recall"
        If Keys(KeyNo).Cancel = False Then
...
'Scale the Display Screen
    ...
    Call ScaleForm(DisplayScreenHeight / Height)
...
'    StatusBar1.Panels(2).Text = ""
    ...
    lblTimeFromLastStart.Visible = False
...
.Enabled = False 'syc
'*        .SetFocus
.BackColor = cbdefault(CommandFromCaption("Postpone"))
        End With
...
'Stop
            ...
            RecallIdx = Classes(LastClassStart(EventTime)).Signal
...
'            End If
            If State.StartClass.Last > 0 Then
                Set SignalAttributes(i).Image = SignalAttributes(Classes(State.StartClass.Last).Signal).Image
            End If
        End If
#End If
...
#If jnasetup Then
    LastCsidx = UBound(Classes)
#Else
    LastCsidx = State.StartClass.Next
#End If
...
'        Call ClearKeyStates     'Must be reset to let the sequence alter the keys
        ...
        Call GoProgramState(4) 'Start Sequence Started
...
'Last Event
    If State.NextEventTime = Evts(UBound(Evts)).ElapsedTime Then
        Call GoProgramState(5) 'Start Sequence Finished
    End If
...
'To next Class start after events
    ...
    If State.Recalls <> 3 Then
        lblTimeFromLastStart = aSecToElapsed(State.NextEventTime - Classes(State.StartClass.Last).start - Classes(State.StartClass.Last).Offset)
    Else
        lblTimeFromLastStart = aSecToElapsed(State.NextEventTime - State.GrStartTime)
    End If
...
'                            Call GoProgramState(4)
'                            Call GoLastClassState(Evts(Eidx).Class)
                            Call GoNextClassStarted
'                            Call LoadStart(Evts(Eidx).Class) called by GoLastClassState
                        Case Else
                        End Select
                    End With
                Next Fcidx
...
'Otherwise do not action the Timeout
    If State.Recalls = 1 Then   'Both so neither selected (no recall)
        If State.StartClass.Last < UBound(Classes) Then
            Call GoSequenceState(2) 'OK to Postpone next class,  Last class has not started
        Else
            Call GoSequenceState(4)  'OK to Finish    Last class has now started
        End If
    Else
...
'Only Called by GoLastClassState each time a class is started
...
Public Function DisplayLastNextStartClass()
    If State.StartClass.Next > 0 Then
...
'Turned on when starttimeisset
    ...
    If State.StartClass.Last > 0 Then
        If State.Recalls <> 3 Then
            lblLastStartName = Classes(State.StartClass.Last).Name
        Else
            lblLastStartName = Classes(State.StartClass.Last).Name & " (recalled)"
        End If
        lblTimeFromLastStart.Visible = True
    Else
        lblLastStartName = "None"
    End If

End Function

'This actually enables/disables the text box
Public Function DisplayFirstStartTime()
...
'Command buttons may not have been defined
    ...
    If FinishIdx > 0 Then
        ...
        End Select
        Commands(FinishIdx).BackColor = cbdefault(FinishIdx)
    End If
    
    PostponeIdx = CommandFromCaption("Postpone")
    If PostponeIdx > 0 Then
        Select Case State.Sequence
'        Case Is = 0, 1, 3 '0=Disabled,1=NotStarted,2=Postpone,3=Recalls,4=Finish
'syc Pospone after start sequence not used
...
'syc            If Keys(KeyNo).State > 0 Then
            If Keys(KeyNo).State > 1 Then
                With StatusBar1.Panels(PanelNo)
                    ...
                    Case Is = 2, 3   'Recall
                        .Text = .Text & " " & Classes(State.StartClass.Last).Name
                    Case Is = 4         'Finish
...
Sub ScaleForm(ScreenScale As Single)
Dim ctrl As Control
Dim myForm As Form
Dim kb As String
...
kb = ctrl.Name
        ...
        If TypeOf ctrl Is ComboBox Then ScalePos = True
        If TypeOf ctrl Is ComboBox Then ExceptHeight = True
        If TypeOf ctrl Is Textbox Then ScalePos = True
...
Dim kb As String
    ...
    If State.Program >= 3 Then  'Start Time set
        kb = "Do you wish to Terminate Racing Signals ?" & vbCrLf
