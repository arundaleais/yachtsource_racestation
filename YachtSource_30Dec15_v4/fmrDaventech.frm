...
Private Function GetVoltage() As String
    Call OpenAndSend("120")
    If UBound(RcvBytes) = -1 Then
        GetVoltage = "Battery Voltage = unavailable"
    Else
        GetVoltage = "Battery Voltage = " & Format$(RcvBytes(0) / 10, "00.0")
    End If
WriteLog GetVoltage & " (" & Timer & ")"
End Function
