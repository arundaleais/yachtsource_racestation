...
Dim KeyNo As Long
    Call WriteLog(Caller & " " & State.NextEventTime)

    Call WriteLog(vbTab & "NextEventTime=" & frmMain.aSecToElapsed(State.NextEventTime))
    Call WriteLog(vbTab & "Program=" & aProgramState(State.Program))
    Call WriteLog(vbTab & "Sequence=" & aSequenceState(State.Sequence))
    Call WriteLog(vbTab & "Recalls=" & aRecallsState(State.Recalls))
    If IsClassesInitialised(Classes) = True Then
        Call WriteLog(vbTab & "NextClassToStart=" & Classes(State.StartClass.Next).Name)
        Call WriteLog(vbTab & "LastClassStarted=" & Classes(State.StartClass.Last).Name)
    End If
    If IsKeysInitialised(Keys) = True Then
        For KeyNo = 1 To UBound(Keys)
            Call WriteLog(vbTab & "Key=" & Keys(KeyNo).KeyName _
